...
export function NavBar() {
  ...
  return (
    <header className="sticky top-0 z-50 h-14 border-b border-border bg-surface/80 backdrop-blur-xl">
      ...
      {mobileOpen && (
        <div className="fixed inset-0 z-50 md:hidden">
          ...
          />
          <nav className="absolute right-0 top-0 flex h-full w-72 flex-col gap-1 border-l border-border bg-surface p-5 shadow-card-lg">
            ...
            <button
              onClick={toggleTheme}
              className="flex items-center gap-2.5 rounded-[var(--radius-md)] px-3.5 py-2.5 text-sm font-medium text-fg-muted transition hover:bg-surface-hover hover:text-fg"
            >
              {theme === "emerald" ? <LeafIcon /> : <SnowflakeIcon />}
            ...
            <Link
              to="/login"
              onClick={() => setMobileOpen(false)}
              className="btn-ghost mt-auto rounded-[var(--radius-md)] px-3.5 py-2.5 text-sm font-semibold"
            >
              Logout
            </Link>
          </nav>
